...
-- Config {{{
-- Dzen/Conky
myXmonadBar = "/usr/local/bin/dzen2 -x '0' -y '0' -h '20' -w \"1366\" -ta 'l' -fg '#FFFFFF' -bg '#000000'"
--myStatusBar = "conky -c ~/.xmonad/.conky_dzen | dzen2 -x '0' -y \"$(expr $(xrandr --current | sed -n '/current/ p' | sed  's/.*current *[0-9]* *x *\\([0-9]*\\).*/\\1/') - 18)\" -w \"1366\" -h '18' -ta 'c' -bg '#000000' -fg '#FFFFFF'"
myBitmapsDir = "~/.xmonad/dzen2"

-- Define Terminal
myTerminal      = "/usr/local/bin/urxvtcd"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    [ 
    -- Programs
      ((modMask,                    xK_p        ), spawn "/usr/local/bin/dmenu_run -fn '-*-arial-*-*-*-*-17-*-*-*-*-*-*-*'") 
    , ((modMask .|. shiftMask,      xK_Return   ), spawn $ XMonad.terminal conf)
    , ((modMask .|. controlMask,    xK_Return   ), spawn "~/.local/bin/st") --Opens up st.
...
--  , ((0,                          0x1008ff13  ), spawn "amixer -q sset Headphone 5%+")   -- XF86AudioRaiseVolume
    ...
    , ((modMask,                    xK_q        ), spawn "killall dzen2 && ~/Library/Haskell/bin/xmonad --recompile && ~/Library/Haskell/bin/xmonad --restart")
    , ((modMask,                    xK_r        ), spawn "~/Library/Haskell/bin/xmonad --restart")
    ]
    ++
